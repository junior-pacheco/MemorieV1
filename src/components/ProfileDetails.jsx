...
const ProfileDetails = () => {
  ...
  console.log('data final', profileData)
  let qrIdentifier = 'hola'

  useEffect(() => {
    const fetchProfileDetails = () => {
      fetch(`http://192.168.0.246:3000/qr-generation?qr=12345`, {
        method: "GET",
        headers: {
        ...
        .then((data) => {
          console.log("Profile data: respuesta", data);
          setProfileData(data);
        })
        .catch((error) => {
  ...
  <div className="bg-[#066165] rounded-lg shadow p-6 mb-6">
    <div className="flex items-center">
      <img
        ...
        className="w-24 h-24 rounded-full mr-6 border-4 border-[#7CB445]"
      />
      <div>
        <h2 className="text-2xl font-bold text-[#7CB445]">Juan Pérez</h2>
        <p className="text-white">1950 - 2023</p>
      </div>
    </div>
    <div className="mt-4 bg-white p-4 rounded shadow">
      <h3 className="text-lg font-semibold text-[#066165] mb-2">
        Biografía
      </h3>
      <p className="text-gray-700">
        Juan Pérez fue una persona excepcional que dejó una huella imborrable en los corazones de quienes lo conocieron.
      </p>
    </div>
  </div>
  )
